...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"%ld-%@",indexPath.row,self.funcArr[indexPath.row]];
    return cell;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *text = self.funcArr[indexPath.row];
    [self addLog:[NSString stringWithFormat:@"%@",text]];
    
    //MARK: 网络接口
    NSUInteger count = self.funcArr.count;
    NSUInteger netCount = 4;
    NSUInteger netIndex = count-indexPath.row;
    if (netIndex <= netCount) {
        ...
        switch ((netCount-netIndex)%netCount) {
            case 0://网络：表盘列表
            {
                /*
                [STNetManager.shared getDialsListWithModel:self.nameModel succeed:^(NSDictionary * _Nonnull dataDict, NSInteger respCode) {
            ...
            case 1://网络：固件版本信息
            {
                [STNetManager.shared getFirmwareInfoWithModel:self.nameModel version:self.version succeed:^(NSDictionary * _Nonnull dataDict, NSInteger respCode) {
                    [self addLog:[NSString stringWithFormat:@"%@",dataDict]];
            ...
            {
                NSLog(@"");
                [STNetManager.shared getUisInfoWithModel:self.nameModel version:self.uiVersion succeed:^(NSDictionary * _Nonnull dataDict, NSInteger respCode) {
                    [self addLog:[NSString stringWithFormat:@"%@",dataDict]];
                } failed:^(NSError * _Nonnull error) {
                }];
            }
    ...
    switch (indexPath.row) {
        ...
        case 36://事件提醒(设置)
        {
            ...
            /**
            NSArray<NSData *> *valueArr = [STBlueToothSender writeAlarmEvens:modelArr DeviceMtu:self.blockReadInterval];
            NSUInteger mtuLength = [STBleManager.sharedInstance.actDeviceModel.peripheral maximumWriteValueLengthForType:CBCharacteristicWriteWithoutResponse];
            for (NSData *blockData in valueArr) {
                ///分MTU包写入
                NSUInteger packetLoction = 0;
                NSUInteger blockLength = blockData.length;
                while (packetLoction < blockLength) {
                    NSUInteger packetLength = 0;
                    if ((packetLoction+mtuLength) >= blockLength) {///最后一包
                        packetLength = blockLength-packetLoction;
                    }else {
                        packetLength = mtuLength;
                    }
                    NSRange packetRange = NSMakeRange(packetLoction, packetLength);
                    NSData *packetData = [blockData subdataWithRange:packetRange];
                    [STBleManager.sharedInstance.actDeviceModel.peripheral writeValue:packetData forCharacteristic:STBleManager.sharedInstance.writeCharacter type:CBCharacteristicWriteWithoutResponse];
                    packetLoction += packetLength;
                }
            }
            */
            return;
...
//            NSString *path = [[NSBundle mainBundle] pathForResource:@"extern_dial_src(4)" ofType:@"bin"];
...
//            NSString *path = [[NSBundle mainBundle] pathForResource:@"bk3633_app_oad" ofType:@"bin"];
//            NSMutableData *data = [[NSMutableData alloc]initWithData:[NSData dataWithContentsOfFile:path]];///获取工程内文件的路径
//
//            NSMutableData *info = NSMutableData.new;
//            Byte type[] = {3};
